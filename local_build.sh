...
if [ -n "${FORCED_VERSION}" ]; then
    echo " - Forced version to ${FORCED_VERSION}"
...
# TODO: first two vars unquoted (and can't be quoted) but what if they have spaces?
# [ -d "${REPO_KOHA_COMMUNITY}" ] && \
#    perl -c ${REPO_KOHA_COMMUNITY} ${REPO_KK_BASE} -I"${SCRIPT_DIR}" "${PM_PATH}" || {
#        echo "Perl syntax check failed!"
#        echo "Run yourself:"
#        echo "    perl -c ${REPO_KOHA_COMMUNITY} ${REPO_KK_BASE} -I\"${SCRIPT_DIR}\" \"${PM_PATH}\""
#        exit 1
#    }
